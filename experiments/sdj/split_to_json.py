...
if __name__=="__main__":
    ...
    df_annotations['area_cut'] = pd.cut(df_annotations['area'],bins= [0,5000,20000,60000,200000,1050000]) #.cut
    ...
    for trn_idx, val_idx in skf.split(df_annotations['id'], df_annotations['category_area'], df_annotations['image_id']):
        if args.split_fold != 0: 
            continue;
    ...
    with open(args.json_save_folder + f'/train_split_{args.split_fold}.json', 'w') as fp:
        json.dump(trn_json, fp)

    with open(args.json_save_folder + f'/valid_split_{args.split_fold}.json', 'w') as fp:
